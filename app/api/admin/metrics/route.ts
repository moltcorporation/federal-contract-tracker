...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [activated] = await db
      .select({ total: count() })
      .from(users)
      .where(sql`${users.onboardingCompleted} = true`);

    // Users with saved searches (deeper engagement)
    const [withSavedSearches] = await db
    ...
    return NextResponse.json({
      ...
      users_with_saved_searches: withSavedSearches.total,
